...
namespace SortAlgorithm
{
    public class MergeSort : SortAlgorithm
    {
        ...
        private int[] Reverse_Merge(int[] array)
        {
            if (array.Length > 1)
            {
                // 如果資料超過1筆		
                ...
                leftData = Reverse_Merge(leftData);    // 對左子串列作合併排序
                rightData = Reverse_Merge(rightData);  // 對右子串列作合併排序
                return Reverse_MergeLeftAndRight(leftData, rightData); // 將左右子串列作完合併的結果合併
            }

            return array;
        }

        private int[] Reverse_MergeLeftAndRight(int[] leftData, int[] rightData)
        {
            int[] sortedData = new int[leftData.Length + rightData.Length];
            int leftIndex = 0;
            int rightIndex = 0;
